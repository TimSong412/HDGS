	...
	float* __restrict__ dL_dcolors)
{
	...
	auto blend_one = [&]() {
		...
		// const float G = (G00 + G10 + G01 + G11 + 8.0f*G_c) / 12.0f;
		...
		{
			dL_ds = {
				// 8.0f/12.0f * dL_dG * -G_c * s.x + dL_dz * Tw.x,
				// 8.0f/12.0f * dL_dG * -G_c * s.y + dL_dz * Tw.y
				// 0.2f * dL_dG * -G_c * s.x + dL_dz * Tw.x,
				// 0.2f * dL_dG * -G_c * s.y + dL_dz * Tw.y
				dL_dG * -G_c * s.x + dL_dz * Tw.x,
				dL_dG * -G_c * s.y + dL_dz * Tw.y
			};
		...
		{
			dL_ds = {
				// 8.0f/12.0f * dL_dG * -G_c * s.x,
				// 8.0f/12.0f * dL_dG * -G_c * s.y,
				// 0.2f * dL_dG * -G_c * s.x,
				// 0.2f * dL_dG * -G_c * s.y
				dL_dG * -G_c * s.x,
				dL_dG * -G_c * s.y
