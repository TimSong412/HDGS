	...
	float* __restrict__ dL_dcolors)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			// if (pix.x == XX && pix.y == YY) {
			// 	printf("id: %d, dL_dc: %f, %f, %f, ", global_id, dchannel_dcolor*dL_dpixel[0]*10000000.f, dchannel_dcolor*dL_dpixel[1]*10000000.f, dchannel_dcolor*dL_dpixel[2]*10000000.f);
			// 	printf("alpha: %f, T: %f\n", alpha, T);
			// }
...
#endif
			// if (pix.x == XX && pix.y == YY) {
			// 	printf("id: %d, dL_dnormal: %f, %f, %f\n", global_id, w*dL_dnormal2D[0]*1000000.f, w*dL_dnormal2D[1]*1000000.f, w*dL_dnormal2D[2]*1000000.f);
			// 	printf("alpha: %f, T: %f, dL_dalph: %f, dL_dG: %f, dL_dz: %f\n\n", alpha, T, dL_dalpha*1000000.f, dL_dG*1000000.f, dL_dz*1000000.f);
			// }
